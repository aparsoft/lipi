...
def split_pdf_through_api(
    ...
    params = {"input_file": input_file, "ranges": ranges, "output_dir": output_dir}
                            ...
                            st.session_state.ranges.append(
                                {
                                    "start": new_start,
                                    "end": new_end,
                                    "name": f"Range{len(st.session_state.ranges)+1}",
                                }
                            )
                        st.rerun()
        ...
        with st.expander(f"Edit {pattern}"):
            ...
            with col1:
                unit_name = st.text_input(
                    "Unit Name",
                    pattern_config.get("unit_name", ""),
                    key=f"unit_{pattern}",
                )
                pattern_config["unit_name"] = unit_name
            with col2:
                prefix = st.text_input(
                    "Prefix", pattern_config.get("prefix", ""), key=f"prefix_{pattern}"
                )
            ...
            for i, range_item in enumerate(pattern_config["page_ranges"]):
                ...
                with col1:
                    start = st.number_input(
                        "Start",
                        value=range_item.get("start", 1),
                        min_value=1,
                        key=f"{pattern}_start_{i}",
                    )
                with col2:
                    end = st.number_input(
                        "End",
                        value=range_item.get("end", 10),
                        min_value=start,
                        key=f"{pattern}_end_{i}",
                    )
                with col3:
                    name = st.text_input(
                        "Name",
                        value=range_item.get("name", ""),
                        key=f"{pattern}_name_{i}",
                    )
                with col4:
